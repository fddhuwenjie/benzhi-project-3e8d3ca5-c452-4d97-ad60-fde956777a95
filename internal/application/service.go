...
type Service struct {
	...
	verifyCache []verificationCacheEntry
}
...
func New(store *storage.Store) *Service {
	return &Service{store: store, evidence: evidence.New(), verifyCache: make([]verificationCacheEntry, 0)}
}
...
func (s *Service) Verify(id string) (evidence.Verification, error) {
	...
	cacheKey := fmt.Sprintf("%s:%d", id, c.Revision)
	for _, cached := range s.verifyCache {
		if cached.key == cacheKey {
			return cached.value, nil
		}
	}
	result := s.evidence.Verify(c, s.store.Events(id))
	s.verifyCache = append(s.verifyCache, verificationCacheEntry{key: cacheKey, value: result})
	return result, nil
}
