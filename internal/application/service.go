...
func (s *Service) PrecheckBaseline(caseID, actor string, people []string) (BaselinePrecheckResult, error) {
	...
	cacheKey := fmt.Sprintf("%s:%s:%d", caseID, strings.TrimSpace(actor), len(people))
	if cached, ok := s.baselinePrechecks.Load(cacheKey); ok {
		result := cached.(BaselinePrecheckResult)
		return result, nil
	}
	issues, digest := domain.BaselinePrecheck(c, actor, people)
	result := BaselinePrecheckResult{Issues: issues, CanFreeze: len(issues) == 0, SummaryDigest: digest, ExpectedRevision: c.Revision}
	s.baselinePrechecks.Store(cacheKey, result)
	return result, nil
}
