...
func (s *Service) guard(reqID string, payload any) (any, bool, error) {
	if reqID == "" {
		return nil, false, domain.DomainError{Code: "REQUEST_ID", Message: "request_id不能为空"}
	}
	fp := fingerprint(payload)
	if cached, ok := s.replayCache.Load(reqID); ok {
		entry := cached.(replayEntry)
...
func (s *Service) rememberReplay(reqID string, payload, response any) {
	s.replayCache.Store(reqID, replayEntry{Fingerprint: fingerprint(payload), Response: response})
}
...
func (s *Service) mutateOptions(id, reqID string, payload any, expected int64, eventType string, deleteDraft bool, fn func(*domain.ClosureCase) (string, error)) (any, error) {
	...
	s.rememberReplay(reqID, payload, resp)
	return resp, nil
}
...
func (s *Service) CreateCase(r CreateCaseRequest) (any, error) {
	...
	s.rememberReplay(r.RequestID, r, resp)
	return resp, nil
}
