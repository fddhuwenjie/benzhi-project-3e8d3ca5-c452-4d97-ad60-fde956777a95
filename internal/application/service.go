...
func (s *Service) SaveLayerDraftContext(ctx context.Context, r DraftRequest) (domain.LayerDraft, error) {
	draft, err := s.SaveLayerDraft(r)
	if err != nil {
		return domain.LayerDraft{}, err
	}
	select {
	case <-ctx.Done():
		return domain.LayerDraft{}, ctx.Err()
	default:
		return draft, nil
	}
}
